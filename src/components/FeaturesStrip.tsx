      ...
      <div className="flex w-[200%] animate-marquee">
        <div className="flex w-1/2 justify-around items-center min-w-max">
          {features.map((feature, idx) => (
            <span key={idx} className={`mx-4 font-sans text-sm uppercase tracking-widest ${feature === "✦" ? "text-[#C9942E] text-lg" : "text-[#F7EBD6] font-semibold"}`}>
              {feature}
            </span>
          ))}
        </div>
        <div className="flex w-1/2 justify-around items-center min-w-max">
          {features.map((feature, idx) => (
            <span key={`dup-${idx}`} className={`mx-4 font-sans text-sm uppercase tracking-widest ${feature === "✦" ? "text-[#C9942E] text-lg" : "text-[#F7EBD6] font-semibold"}`}>
              {feature}
            </span>
          ))}
        </div>
